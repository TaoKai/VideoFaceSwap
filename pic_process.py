...
def innerGen68(path):
    ...
    src = landmarks_2D*np.array([128, 128])+np.array([24, 16])
...
class FaceData(object):
    ...
    def getTestBatch(self):
        ...
        for l in lst:
            ...
            if img is not None:
                img = cv2.warpAffine(img, M, (176, 168), borderValue=0.0)
                imgs.append(img)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        wa = warpsA.detach().numpy().transpose(0, 2, 3, 1).reshape(-1, 176, 3)*255
        wa = wa.astype(np.uint8)
        ia = imgsA.detach().numpy().transpose(0, 2, 3, 1).reshape(-1, 176, 3)*255
        ia = ia.astype(np.uint8)
        wb = warpsB.detach().numpy().transpose(0, 2, 3, 1).reshape(-1, 176, 3)*255
        wb = wb.astype(np.uint8)
        ib = imgsB.detach().numpy().transpose(0, 2, 3, 1).reshape(-1, 176, 3)*255
