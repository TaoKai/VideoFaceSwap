...
class Color():
    ...
    """
    def process(self, old_face, new_face, raw_mask=None):
        """
        Parameters:
        -------
        source: NumPy array
        ...
        if raw_mask is None:
            raw_mask = np.ones(old_face.shape[:2], dtype=np.float32)
        raw_mask = raw_mask.reshape(raw_mask.shape+(1,))
