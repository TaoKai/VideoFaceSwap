...
class SAE_RESNEXT_ENCODER(nn.Module):
    ...
    def forward(self, x):
        ...
        return ups_out, l1_out, l2_out, l3_out, l4_out
...
class SAE_DECODER_UNET(nn.Module):
    def __init__(self):
        super(SAE_DECODER_UNET, self).__init__()
        self.db1 = _DecoderBlock(512, 256)
        self.db2 = _DecoderBlock(256, 128)
        self.db3 = _DecoderBlock(128, 64)
        self.db4 = _DecoderBlock(64, 32)
        self.conv = nn.Conv2d(32, 3, 1)
        self.activ = nn.Sigmoid()
        self.ub1 = _UnetBlock(2048, 256)
        self.ub2 = _UnetBlock(1024, 128)
        self.ub3 = _UnetBlock(512, 64)
        self.ub4 = _UnetBlock(256, 32)
    
    def forward(self, x, l1, l2, l3, l4):
        x4 = self.db1(x)
        x4 = self.ub1(x4, l4)
        x3 = self.db2(x4)
        x3 = self.ub2(x3, l3)
...
class _UnetBlock(nn.Module):
    ...
    def forward(self, x, l):
        l = self.conv(l)
        l = F.upsample(l, size=x.shape[2:], mode='bilinear')
        x = x+l
        x = self.bn(x)
        x = self.relu(x)
        return x
...
class AutoEncoderUnet(nn.Module):
    def __init__(self):
        ...
        self.decoder = SAE_DECODER_UNET()
    
    def forward(self, x):
        identity = x
        x, l1, l2, l3, l4 = self.encoder(x)
        x = self.decoder(x, l1, l2, l3, l4)
