...
#     print(kernel, kernel.shape)
    ...
    # 单独提取每一个通道
    out_0 = F.conv2d(img[:, :1, ...], weight, padding=int((kernel_n - 1) / 2)).to(device)
    out_1 = F.conv2d(img[:, 1:2, ...], weight, padding=int((kernel_n - 1) / 2)).to(device)
    out_2 = F.conv2d(img[:, 2:3, ...], weight, padding=int((kernel_n - 1) / 2)).to(device)
...
class Loss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        costl1 = self.l1(y_pred, y_true)
        costdssim = self.dssim(y_pred, y_true)
        cost = costdssim*0.6+costl1*0.4
        return cost
